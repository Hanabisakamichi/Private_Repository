...
for path,dirlist,filelist in os.walk(dir):
	for file in filelist:
		...
		if os.path.splitext(file)[1] == '.txt':
			...
			df = pd.read_csv(file_path)
			print('draw',file_path)
			sd_cpu_invoke = data_smooth(df[' CPU_Invoke_Time(ms)'],10)

			plt.plot(CPU_scheduling,color='firebrick',linewidth=1)
			plt.legend()
			plt.title('CPU Invoke')
			plt.xlabel('Time')
			plt.ylabel('CPU Invoke Time(μs)')
			plt.set_ylim(0, 5000)
			plt.savefig(path+'/log_figure/'+file+'_cpu.png',dpi=1000)
			plt.close()
